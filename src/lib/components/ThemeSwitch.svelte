<script lang="ts">
  import {theme, AllowedOptions} from "$lib/stores/theme";
  import {Listbox, ListboxButton, ListboxOption, ListboxOptions} from "@rgossiaux/svelte-headlessui";
  import Icon from "@iconify/svelte";
  import {browser} from "$app/environment";

  const themeIcons: Record<typeof AllowedOptions[number], string> = {
    ...
    <div class="absolute w-36 top-0">
        <ListboxButton
            ...
            <span class="flex items-center">
                <Icon icon={themeIcons[$theme]} class="h-5 w-5 flex-shrink-0"/>
                <span class="ml-3 block truncate capitalize">{$theme}</span>
            </span>
        ...
        <ListboxOptions
            ...
            {#each AllowedOptions as option (option)}

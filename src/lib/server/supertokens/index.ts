...
import type { Tokens } from './cookie';
...
const SuperTokensHelpers = {
	async signin(email: string, password: string): Promise<SignInResult> {
		const signinResult = await EmailPassword.signIn('public', email, password);
	...
	async emailExists(email: string): Promise<boolean> {
		return false;
		// TODO: https://supertokens.com/docs/emailpassword/common-customizations/get-user-info
	...
	async refreshToken({ refreshToken, antiCsrfToken }: TokensForRefresh): Promise<Partial<Tokens>> {
		try {
			...
			if (error.payload.sessionHandle) await Session.revokeSession(error.payload.sessionHandle);
			return {};
		}
	},
